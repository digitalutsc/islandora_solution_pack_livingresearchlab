...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="foxml:datastream[@ID='DWC']/foxml:datastreamVersion[last()]"
            ...
            <xsl:choose>
                <xsl:when test="contains(text(), 'purchasedFrom')">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of
                                    select="concat($prefix, 'purchasedFrom')"/>
                        </xsl:attribute>
                        <xsl:value-of
                                select="substring-after(text(), 'purchasedFrom ')"
                                />
                    </field>
                </xsl:when>
                <xsl:when test="not(contains(name(), 'basisOfRecord'))">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of
                                    select="concat($prefix, substring-after(name(),':'))"
                                    />
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
            ...
            <xsl:choose>
                <xsl:when test="not(contains(name(), 'taxonRemarks'))">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of
                                    select="concat($prefix, 'Taxon_', parent::node()/*[contains(name(), 'taxonRemarks')]/text())"
                                    />
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
        ...
        <xsl:for-each select="$content//dcterms:Location/*">
            <xsl:choose>
                <xsl:when
                        test="not(contains(name(), 'locality')) and not(contains(name(), 'locationRemarks'))">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of
                                    select="concat($prefix, translate(parent::node()/*[contains(name(), 'locality')]/text(), ' ', ''), '_location')"
                                    />
                        </xsl:attribute>
                        <xsl:value-of
                    ...
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of
                                    select="concat($prefix, translate(parent::node()/*[contains(name(), 'locality')]/text(), ' ', ''), '_location_PID')"
                                    />
                        </xsl:attribute>
                        <xsl:value-of
            ...
            <xsl:choose>
                <xsl:when test="not(contains(name(), 'eventRemarks'))">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of
                                    select="concat($prefix, parent::node()/*[contains(name(), 'eventRemarks')]/text(), '_', substring-after(name(),':'))"
                                    />
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
                ...
                <xsl:when test="not(contains(name(), 'relationshipOfResource'))">
                    <xsl:if test="contains(name(), 'relatedResourceID')">
                        <field>
                            <xsl:attribute name="name">
                                <xsl:value-of
                                        select="concat($prefix, 'Relationship_', parent::node()/*[contains(name(), 'relationshipOfResource')]/text(), '_PID')"
                                        />
                            </xsl:attribute>
                            <xsl:value-of
                        ...
                        <field>
                            <xsl:attribute name="name">
                                <xsl:value-of
                                        select="concat($prefix, 'Relationship_', parent::node()/*[contains(name(), 'relationshipOfResource')]/text())"
                                        />
                            </xsl:attribute>
                            <xsl:value-of
                                    select="normalize-space(substring-before(text(),'('))"
                    ...
                    <xsl:if test="not(contains(name(), 'relatedResourceID'))">
                        <field>
                            <xsl:attribute name="name">
                                <xsl:value-of
                                        select="concat($prefix, 'Relationship_', substring-after(name(), ':'))"
                                        />
                            </xsl:attribute>
                            <xsl:value-of select="."/>
                        </field>
